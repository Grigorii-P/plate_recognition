...
def match_plates(new_coord, coords, cur_text, target_list, cur_frame, pl_offset):

    # match_plates( zone_center, coords, text, frame_count )

    # if found close elems - print its box
    to_del = []
    for coord in coords.keys():
        if is_close(coord, new_coord, pl_offset):
            # if we encounter a plate
            # that disappeared quite a bit ago
            ...
            text = coords[coord][1]
            del coords[coord]
            coords[new_coord] = [cur_frame, text]
            return True, text
    ...
    check_text_dists = [x for x in target_list if distance(
        cur_text, x) <= LEV_THRESHOLD]
    if any(check_text_dists):
        text = check_text_dists[0]
        coords[new_coord] = [cur_frame, text]
        return True, text
...
def video_inference(src, dst, target_list, pl_offset):
    ...
    while True:
        ...
        for img_mask in cv_img_masks:
            ...
            is_match, correct_text = match_plates(
                zone_center, coords, text, target_list, frame_count, pl_offset)
            ...
            if is_match:
                ...
                if len(text) >= TRASH_FILTER:
                    print_line_and_text(img, points, correct_text, RED_COLOR)
...
if __name__ == '__main__':
    ...
    REGION_MODEL_PATH = os.path.join(
        NOMEROFF_NET_DIR, '/models/imagenet_vgg16_np_region_2019_1_18.h5')
    ...
    # regionDetector.load(REGION_MODEL_PATH)2

    path_src = 'video_results/preprocessed'
    path_dst = 'video_results/result/'
    ...
    for video in videos:
        ...
        t0 = time()
        try:
            video_inference(join(path_src, video), join(path_dst, video), videos[video], PLATE_OFFSET)
